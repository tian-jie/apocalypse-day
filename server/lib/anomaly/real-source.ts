      ...
      return {
        ...
        ingestion: {
          ...
          sourceStatus: freshness === 'fresh' ? 'healthy' : 'degraded',
            syncStatus: 'healthy',
            dataState: snapshots.length >= 3 ? 'ready' : 'insufficient',
          freshness,
          ...
          persistedAt: new Date().toISOString(),
            syncFailureReason: freshness === 'stale' ? 'Provider data exceeded freshness threshold.' : undefined,
            notes: freshness === 'stale' ? ['Provider data is stale.'] : [],
        },
      }
    }
    finally {
